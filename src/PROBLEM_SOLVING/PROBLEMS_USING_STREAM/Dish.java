...
public class Dish {
    ...
    public static void main(String[] args) {
        ...
        List<Dish> menu = Arrays.asList(new Dish("Salad", 150.0f, 4.5f, 200.0f, Type.VEG),
                new Dish("Burger", 300.0f, 3.5f, 500.0f, Type.NONVEG),
                new Dish("Fries", 120.0f, 4.0f, 350.0f, Type.VEG),
                new Dish("Pasta", 200.0f, 4.5f, 400.0f, Type.VEG),
                new Dish("Pizza", 250.0f, 5.0f, 300.0f, Type.VEG));

        // Stream to filter and group dishes
        Map<Float, List<Dish>> groupedDishes = menu.stream()
                .filter(dish -> dish.getTypeOfDish() == Type.VEG)
                .filter(dish -> dish.getCalories() < 400)
                .filter(dish -> dish.getRatings() >= 4.0f)
                .collect(Collectors.groupingBy(dish -> dish.getRatings()));
//                .collect(Collectors.groupingBy(Dish::getRatings));
        ...
        Map<Type, Map<Float, Double>> groupedCalories = menu.stream()
        ...
        Map<Type, List<Dish>> groupedDishesType = menu.stream().collect(Collectors.groupingBy(Dish::getTypeOfDish));
